...
// console.log(sentiment.positivity(str).score,sentiment.positivity(str).words); // console.log(sentiment.negativity(str).score,sentiment.negativity(str).words); // console.log(JSON.stringify(sentiment.analyze(str))); 
...
// bayes.probabilities(); 
// let results = bayes.guess("Yesterday, I was very very happy, so happy."); // console.log(JSON.stringify(results)); 
...
new Chart(document.getElementById("myChart"), { 
 ...
 datasets: [ 
 { 
 backgroundColor: ["#3e95cd", "#8e5ea2","#ffffff"],  data: [ 
 sentiment.positivity(str).score,  sentiment.negativity(str).score,  sentiment.analyze(str).score  ] 
 } 
 ] 
 }, 
 options: { 
...
new Chart(document.getElementById("myChart1"), {  type: 'bar', 
 data: { 
 labels: ["POSITIVE", "NEGATIVE",'OVERALL SCORE'],  datasets: [{ 
  
 backgroundColor: ["#3e95cd", "#8e5ea2","#ffffff"],  data: [ 
 sentiment.positivity(str).comparative,  sentiment.negativity(str).comparative,  sentiment.analyze(str).comparative  ] 
 }, 
 ...
 yAxes: [{ 
 display: true,  ticks: { 
 beginAtZero: true,  steps: 0.5,  stepValue: 0.5,  max:5, 
 min:-5 
 } 
 }]  
 }, 
